#!/usr/bin/env python3
"""运行 CLM v0.2 核心回归 Gate。"""
from __future__ import annotations
import json, subprocess, sys, tempfile
from pathlib import Path

ROOT=Path(__file__).resolve().parent.parent
SCRIPTS=ROOT/"scripts"; FIXTURES=ROOT/"evals"/"fixtures"; SCHEMA=ROOT/"schemas"/"clm-v0.2.schema.json"
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="logic-v02-reg-") as td:
        out=Path(td)
        checks.append(run([SCRIPTS/"validate_clm.py",model,"--schema",SCHEMA],"v0.2 合法模型通过校验"))
        checks.append(run([SCRIPTS/"render_human_logic.py",model,"-o",out/"human.md"],"中文逻辑投影"))
        checks.append(run([SCRIPTS/"generate_test_vectors.py",model,"--output",out/"tests.json"],"测试向量生成"))
        ...
        checks.append(run([SCRIPTS/"compile_iir.py",model,profile,"-o",out/"iir.json"],"IIR 编译"))
        ...
        if (out/"changed.json").exists():
            checks.append(run([SCRIPTS/"validate_clm.py",out/"changed.json","--schema",SCHEMA],"变更后模型重新校验"))
        ...
        if (out/"tests.json").exists():
            data=json.loads((out/"tests.json").read_text(encoding="utf-8")); semantic_ok=any(v.get("kind")=="scenario" and isinstance(v.get("given"),dict) and isinstance(v.get("expect"),dict) for v in data.get("vectors",[]))
        checks.append({"label":"Typed Scenario 标准测试向量","ok":semantic_ok,"returncode":0 if semantic_ok else 1,"stdout":"","stderr":""})
    ok=all(x["ok"] for x in checks); print(json.dumps({"ok":ok,"checks":checks},ensure_ascii=False,indent=2)); return 0 if ok else 1
