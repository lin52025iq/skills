...
function pretty(value){
  if(value&&typeof value==='object'&&value.ref)return value.ref;
...
function renderStep(ref,index,depth=0){
  ...
  if(node.kind==='decision'){
    const lines=[`${pad}如果 ${renderExpression(node.when)}：`];
    for(const x of node.then??[])lines.push(renderStep(x,index,depth+1));
    if((node.else??[]).length){lines.push(`${pad}否则：`);for(const x of node.else)lines.push(renderStep(x,index,depth+1));}
    return lines.join('\n');
  }
  if(node.kind==='foreach'){
    const lines=[`${pad}遍历 ${renderValue(node.collection)}，当前项记为 ${node.item}${node.when?`，仅处理满足「${renderExpression(node.when)}」的项`:''}：`];
    for(const x of node.do??[])lines.push(renderStep(x,index,depth+1));
    return lines.join('\n');
  }
...
function renderHuman(document){
  ...
  for(const behavior of root.behaviors??[]){
    ...
    if((behavior.flow??[]).length){chunks.push('','### 处理过程');let n=1;for(const ref of behavior.flow){const text=renderStep(ref,index);const lines=text.split('\n');chunks.push(`${n}. ${lines[0]}`,...lines.slice(1).map(x=>`   ${x}`));n++;}}
    chunks.push('');
  }
  for(const scenario of root.scenarios??[]){
    chunks.push(`## 场景：${scenario.name??scenario.id}`,'');
    for(const a of scenario.given??[])chunks.push(`- 已知：${renderValue(a.target)} = ${renderValue(a.value)}`);
    chunks.push(`- 当：${(scenario.when??[]).join('、')}`);
    for(const a of scenario.then??[]){if(a.target&&a.value)chunks.push(`- 则：${renderValue(a.target)} = ${renderValue(a.value)}`);else if(a.expression)chunks.push(`- 则满足：${renderExpression(a.expression)}`);}
    chunks.push('');
  }
  return chunks.join('\n');
}
  ...
  for(const node of index.values()){
    if(node.kind==='rule'&&['in','not_in'].includes(node.expression?.op)){
      ...
      if(subject&&Array.isArray(declared)){
        const expectedDeclared=node.expression.op==='in';
        for(const value of declared)vectors.push({id:`test.${node.id}.declared.${value}`,source_semantic_id:node.id,kind:expectedDeclared?'rule_positive':'rule_negative',given:{[subject]:value},when:null,expect:{rule_result:expectedDeclared}});
        const type=symbols[subject]?.type,universe=symbols[type]?.enum_values??[];
        for(const value of universe.filter(x=>!declared.includes(x))){const expected=node.expression.op==='not_in';vectors.push({id:`test.${node.id}.complement.${value}`,source_semantic_id:node.id,kind:expected?'rule_positive':'rule_negative',given:{[subject]:value},when:null,expect:{rule_result:expected}});}
      }
    }else if(node.kind==='rule'&&node.expression?.op){
      vectors.push({id:`test.${node.id}.expression`,source_semantic_id:node.id,kind:'rule_expression_intent',given:{expression:node.expression},when:null,expect:{evaluate_expression:true}});
    }
    if(node.kind==='scenario'){
      const map=(items)=>Object.fromEntries((items??[]).filter(x=>x.target&&x.value).map(a=>[valueRef(a.target),unwrapValue(a.value)]).filter(([k])=>k));
      vectors.push({id:`test.${node.id}.example`,source_semantic_id:node.id,kind:'scenario',given:map(node.given),when:{behaviors:node.when??[]},expect:map(node.then)});
    }
...
function verifyManifest(directory){
  ...
  for(const artifact of manifest.artifacts??[]){
    ...
    if(!fs.existsSync(file)){errors.push({code:'GENERATED_FILE_MISSING',path:artifact.path});continue;}
...
try{
  if(command==='symbols'){
    const output=option(args,'-o','--output');if(!output)fail('symbols 需要 -o/--output');writeJson(output,{symbols:buildSymbolTable(readJson(args[0]))});
  }else if(command==='hash')console.log(semanticHash(readJson(args[0])));
  else if(command==='render'){
    const text=renderHuman(readJson(args[0])),output=option(args,'-o','--output');if(output){fs.mkdirSync(path.dirname(output),{recursive:true});fs.writeFileSync(output,text,'utf8')}else console.log(text);
  }else if(command==='test-vectors'){
    const result=testVectors(readJson(args[0])),output=option(args,'-o','--output');if(output)writeJson(output,result);else console.log(JSON.stringify(result,null,2));
  }else if(command==='verify-manifest'){
    const result=verifyManifest(args[0]);console.log(JSON.stringify(result,null,2));process.exit(result.valid?0:1);
