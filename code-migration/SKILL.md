...
凡是会复制或生成到目标仓库 `.code-migration/` 的标准模板，都必须使用稳定的两位前置序号。编号按**使用 / 产出顺序**排列，不按模板创建历史排列：
...
01–02    = 任务与执行约束
03       = 范围和进度权威台账
04–06    = 语义、行为、交互与页面结构契约
07–08    = 多源融合与重大目标决策
09       = 基于前述事实形成的执行调度
```

不是每次迁移都必须生成 `00–09` 全部文件；风险和任务类型决定实际产物。但**只要使用标准模板落地到目标工作区，就保持该标准编号，不临时改号或创建无序号同类文档。** 模块局部自定义说明放在 `modules/<module>/`，不与顶层标准编号竞争。

## 3. 权威文档边界

```text
02 = 长期迁移约束 / 开发规则
03 = 范围 / Feature / optional Group / Atomic Item / 实施状态
04 = 可选复杂业务语义，不维护进度
05 = Source Behavior / Target 业务语义 / 行为验证（WHAT）
06 = 页面结构 + 交互体验（HOW USER EXPERIENCES IT + WHERE）
07 = 多来源功能归一与冲突映射
08 = 重大迁移决策 / 已批准变化
...
Feature 只有在全部范围内必需 Atomic Item 完成、要求的 Discovery / `05 / 06` 验证通过、必要集成验证通过后才能 `已迁移`。
...
不强制 Group、`04 / 05 / 06`、输入基线、Workstream 编排或 Batch。

### 中风险

按需要增加：关键 Discovery、关键 SB、关键 INT / REG、可选 Group、可选 `04` 和长任务输入基线。只为值得独立迁移追踪的内容建立 ID。

### 高风险

按需要启用完整 Discovery、`04 / 05 / 06`、C3 运行证据、输入基线、独立验证、并行安全、集成验证、回滚和清理。

## 8. Source Behavior 与交互 / 结构边界
...
简单任务直接实施/验证 Atomic Item；只有多个强关联项确实需要一起处理时才创建临时 DEV/VER Batch。Batch 完成后必须逐 Atomic Item 回写状态，Batch 自身不是长期事实源。
...
- 不把用户批准的新方案修回 Source；
- 不创建与 `00–09` 标准用途重复但编号不同或无编号的顶层迁移文档。

## 15. 结束报告

至少说明：完成能力、Atomic Item / Feature / Module 派生状态、来源真实性、关键 Discovery、用户新方案、`05 / 06` 验证、必要集成验证、阻塞/未知和下一安全动作。
